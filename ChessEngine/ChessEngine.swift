...
extension Collection where Iterator.Element == Piece {
    var valuation: ChessEngine.Valuation {
        return self.lazy.filter { !$0.kind.isKing }.map { $0.kind.relativeValue }.reduce(0, +)
    }
}
...
fileprivate extension Board {
    ...
    fileprivate func piecesOutsideOriginalPosition(for color: Color) -> Int {
...
final class ChessEngine {
    ...
    let game = Game(whitePlayer: Player(kind: .human), blackPlayer: Player(kind: .computer), variant: Variant.standard)
