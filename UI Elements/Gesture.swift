...
class SingleFingerGesture: Gesture {
	...
	func finishGesture() {
		...
		for result in results {
            if let tappedSquare = result.node as? TappableNode {
                // Move piece to the selected area
                GameManager.move(tappedSquare: tappedSquare)
                print("Square tapped at \(tappedSquare.location.file) \(tappedSquare.location.rank)")
            } else if let tappedPiece = result.node as? PieceNode {
                // Another piece has been selected, clear the pieces and add new pieces
                GameManager.move(tappedPiece: tappedPiece)
                print("Piece tapped at \(tappedPiece.location.file) \(tappedPiece.location.rank)")
            }
		}
    }
