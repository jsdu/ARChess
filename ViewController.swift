...
class ViewController: UIViewController, ARSCNViewDelegate, UIPopoverPresentationControllerDelegate, VirtualObjectSelectionViewControllerDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        GameManager.shared.game = Game()
        GameManager.shared.game.blackPlayer.kind = .computer
	...
	func loadVirtualObject(at index: Int) {
		...
		DispatchQueue.global().async {
            ...
            if let object = object as? ChessBoard {
                object.loadModel(board: GameManager.shared.game.board)
            } else {
                object.loadModel()
            }
